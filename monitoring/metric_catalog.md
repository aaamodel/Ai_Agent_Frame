...
> 这份文件是「面试第 10 题」的答案稿：**线上该看哪些指标、每个指标怎么算、数据从哪来、多少该报警**。
>
...
面试时**主动说这条**，比被追问出来强得多——它体现你清楚自己系统的边界在哪。
...
面试时被问到「你监控做得全吗」，正确回答是：

> 「我分四层列了指标字典，其中性能的成本的稳定性的一部分已经跑通并有真实数字，
> 另一部分因为缺埋点我先空着，并且写清了缺什么、补的代价多大。
> 我认为**列出一个自己都知道没采集的指标、还给它编个数字**，是最差的做法。」
...
*本文件中的价格为量级参考，实际请核实厂商实时定价。压测类数字若写入简历，必须标注「本机单机环境，非生产环境数据」。*
