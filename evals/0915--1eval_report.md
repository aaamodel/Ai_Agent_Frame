...
- rag.mrr_min 违规：mrr=0.5000 < 0.5500
- cost.avg_tokens_per_turn_max 违规：avg_tokens_per_turn=36217.0000 > 6000.0000
...
| RAG MRR | 0.5 ❌ | 0.55 | `rag.mrr_min` |
| 工具调用成功率 | 1 ✅ | 0.75 | `tool.call_success_min` |
| 工具关键参数命中率 | 1 ✅ | 0.6 | `tool.key_arg_recall_min` |
| 单轮 token 成本（端到端） | 36217 ❌ | 6000 | `cost.avg_tokens_per_turn_max` |
...
- `mean_latency_ms` = 19536.3800
- `median_tokens_per_turn` = 3427.0000
- `output_tokens` = 0
- `p95_latency_ms` = 22649.1560
- `total` = 2
- 样本数 = 2
- 真实 LLM 调用 2 次，input=6227 output=627 total=6854 tokens
...
- `hit@5` = 1.0000
- `mean_latency_ms` = 2765.6350
- `mrr` = 0.5000
- `p95_latency_ms` = 3571.4545
- `recall@5` = 1.0000
- `scored` = 1
...
| id | 问题 | 期望 | 实际 |
| --- | --- | --- | --- |
| R28 | 我们公司 2027 全年的营收目标是多少？ |  | top1=异议处理与竞争话术库 |

### 3.3 工具调用

- `avg_tokens_per_turn` = 36217.0000
- `error_rate` = 0.0000
- `key_arg_cases` = 2
- `key_arg_recall` = 1.0000
- `llm_calls_per_request` = 12.0000
- `mean_latency_ms` = 168429.9000
- `median_tokens_per_turn` = 36217.0000
- `memory_cleanup_failures` = 0
- `memory_isolated` = True
- `p95_latency_ms` = 272082.6480
- `tool_success_rate` = 1.0000
- `total` = 2
- 样本数 = 2
- 真实 LLM 调用 24 次，input=66138 output=6296 total=72434 tokens
...
- **阶段**：RAG 检索
- **用例**：`R28` —— 我们公司 2027 全年的营收目标是多少？
- **期望**：
- **实际**：top1=异议处理与竞争话术库（score=0.030415） 召回列表=['异议处理与竞争话术库', '销售方法论与商机资质判定手册', '销售目标与考核', '销售方法论与商机资质判定手册', '销售目标与考核']
- **实际召回的 top1 内容开头**：### 3.3 "今年没预算了" - 不强推。转入 nurture 名单，确认明年预算周期（Q4 立项、Q1 执行是常见节奏），设 90 天回访； - 记录到台账"下一步动作"，不要让线索静默死亡（对应输单原因"预算冻结"——岭南文旅、湘江

> 讲法建议：先说「期望什么」，再说「实际发生了什么」，最后说「我认为根因是什么、下一步怎么验证」——这三点比结论本身更有信息量。

## 五、覆盖范围与已知局限

- 黄金集规模：意图 22 条 / RAG 32 条 / 工具 17 条。
- ``--limit`` 为**分层抽样**（见 ``evals/sampling.py``），不是'取前 N 条'：必含边界样本 / 应拒答样本，其余在各层间轮转分配。因此**冒烟跑的数字不能代表全量**，只能验证链路通不通。
